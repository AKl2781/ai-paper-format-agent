...
当前稳定展示基线：tag `v1.0-showcase`，指向 commit `10904db`。`main` 分支在该 tag 之后继续补充公开前文档和面试材料；截至本轮公开前清理开始时，`main` 为 `afed51e docs: polish interview showcase materials`，后续提交可能继续前进，但不代表 `v1.0-showcase` tag 已移动。
...
- v1.0-showcase 稳定展示版：tag `v1.0-showcase` 指向 `10904db`；`main` 分支包含 tag 之后的公开前文档和面试材料补充。
...
当前推荐演示基线：tag `v1.0-showcase`，对应 commit `10904db`。

版本口径：

- `v0.9.2-ui-fetch-compat-fix`：早期稳定演示代码基线，已通过 final demo check。
- `v0.9.4-demo-screenshot-package`：上一阶段截图/录屏素材包 tag。
- `v1.0-showcase`：稳定展示版 tag，指向 `10904db`。
- `main`：包含 `v1.0-showcase` 之后的公开前文档和面试材料补充；截至本轮公开前清理开始时为 `afed51e` 或更新后的提交。
...
- 图表编号和正文引用检查。
- 重复风险检测 / 相似度预检。
- local 模式：本地格式规则，不启用 AI 评分，保持 `ai_score=null`、`ai_used=false`。
- ai 模式：在格式修复基础上尝试 AI 语言审校；LLM 不可用时 fallback 到本地规则，不中断主流程。
- `agent_trace` 标准化执行轨迹。
...
`manifest.csv`、`generated_manifest.csv`、`heavy_manifest.csv` 对应的完整 DOCX 回归样本属于本地脱敏样本和压力测试资产，不全部随公开仓库发布。完整 manifest / heavy 回归不是公开 clone 后的默认必跑流程；如需复现生成样本回归，可先使用 `generate_expanded_test_corpus.py` 生成本地样本，再按需运行 `run_real_doc_regression.py`。
...
- 运行时模板副本：
  - `demo_inputs/template_sample.docx` 是固定 demo 输入样本，会被 Git 跟踪。
  - `paper-ai/backend/templates/*.docx` 是上传模板在后端运行时产生的副本，不应提交到 Git；v0.8.6 已通过 `.gitignore` 忽略这类运行产物。
- `docs/DEMO_CASE.md`
...
本项目采用 MIT License。本项目用于学习、实习投递和技术展示，示例数据仅用于演示。
...
- [截图/录屏素材指南](docs/DEMO_SCREENSHOT_GUIDE.md)
- [v1.0-showcase 封版摘要](docs/V1_0_SHOWCASE_SUMMARY.md)
